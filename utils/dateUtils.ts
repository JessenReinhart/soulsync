...
export const getFormattedDate = (dateInput: string | Date | null | undefined, options?: Intl.DateTimeFormatOptions): string => {
  if (!dateInput) { // Handles null, undefined, or empty string
    return "Invalid Date";
  }
   ...
   if (isNaN(date.getTime())) { // Check if date is valid
    ...
    if(typeof dateInput === 'string' && dateInput.match(/^\d{4}-\d{2}-\d{2}$/)) {
        ...
        if (!isNaN(fixedDate.getTime())) {
            return fixedDate.toLocaleDateString(undefined, options || { year: 'numeric', month: 'long', day: 'numeric' });
        }
    }
    return "Invalid Date";
  }
